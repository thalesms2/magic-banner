...
Crie o Bucket banner_images no Supabase Storage e configure-o como Público.
...
NEXT_PUBLIC_SUPABASE_ANON_KEY="[SUA_CHAVE_ANON_PÚBLICA]"
```
...
Crie um arquivo HTML local (teste.html) e simule a URL alvo. Você pode usar uma extensão de navegador para alterar a URL ou simplesmente carregar o arquivo local.
...
<body>
    <h1>Página Principal da Loja</h1>
...
Ao carregar o teste.html, o script será executado, fará a requisição para a API (/api/banners?url=file:///... ou a URL que você definir), e injetará o banner no topo se as condições (URL e horário) forem atendidas.
